...
def animate(i, xs, accx, accy, gyrox, gyroy, fcx, fcy, fkx, fky):
    ...
    dFcx = float(line_as_list[5])
    dFcy = float(line_as_list[6])
    dFkx = float(line_as_list[7])
    temporalFky = line_as_list[8]
    temporalFkyAsList= temporalFky.split(b'\n')
